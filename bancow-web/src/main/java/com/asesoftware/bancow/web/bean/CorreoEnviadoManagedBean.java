...
@ManagedBean
@ViewScoped
public class CorreoEnviadoManagedBean implements Serializable {
	...
	private static final Logger logger = Logger.getLogger(CorreoEnviadoManagedBean.class.getName());
	
	private CorreoEnviado correoEnviado;
	private List<CorreoEnviado> correoEnviadoList;
    ...
    public void search() {
    	try {	
    		...
    		this.correoEnviadoList = this.consultar(this.searchExpression);
            FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, "Busqueda", "Mostrando "+ this.correoEnviadoList.size() + " registro(s).");
     ...
     * @return
     */
    public List<CorreoEnviado> consultar(SearchExpression search) {
        return negocioCorreoEnviado.consultar(search);
    }
    ...
    private void setFieldDomainList() {
    	this.fieldList = new ArrayList<String>(Arrays.asList(CorreoEnviado.getAtributosEntidadCorreoEnviado()));
		...
		this.fieldDomainList.add(new SelectItem("correoMail","correoMail"));
		this.fieldDomainList.add(new SelectItem("fecha","fecha"));

    }
    
    public void createCorreoEnviadoWithForm() {
    }
     ...
     * @param correoEnviado
     */
    public void deleteCorreoEnviado(CorreoEnviado correoEnviado) {
	    try{
	    	negocioCorreoEnviado.delete(correoEnviado);
	    	FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_WARN, "Eliminar CorreoEnviado", "Idenfitificado por: " +
	    		"arcProcCodigoProceso: " + correoEnviado.getCorreoEnviadoPK().getArcProcCodigoProceso()+ ", "+	    		"correoMail: " + correoEnviado.getCorreoEnviadoPK().getCorreoMail());
	        FacesContext.getCurrentInstance().addMessage(null, message);
	        this.search();
        }
        catch (Exception e){
    ...
    public void onRowEdit(RowEditEvent event) {
	    try{
	        FacesMessage msg = new FacesMessage("CorreoEnviado Editado");
	        negocioCorreoEnviado.edit((CorreoEnviado) event.getObject());
    ...
    public void onRowCancel(RowEditEvent event) {
        FacesMessage msg = new FacesMessage("CorreoEnviado Cancelado");
        FacesContext.getCurrentInstance().addMessage(null, msg);
    }
     ...
     * Getters AND Setters
     */
    public List<CorreoEnviado> getCorreoEnviadoList() {
        return correoEnviadoList;
    }
