...
@ManagedBean
@ViewScoped
public class ConvenioManagedBean implements Serializable {
	...
	private static final Logger logger = Logger.getLogger(ConvenioManagedBean.class.getName());
    ...
    public void search() {
    	try {	
    		...
    		this.convenioList = this.consultar(this.searchExpression);
            FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, "Busqueda", "Mostrando "+ this.convenioList.size() + " registro(s).");
     ...
     * @return
     */
    public List<Convenio> consultar(SearchExpression search) {
        return negocioConvenio.consultar(search);
    }
    ...
    private void setFieldDomainList() {
    	this.fieldList = new ArrayList<String>(Arrays.asList(Convenio.getAtributosEntidadConvenio()));
		this.fieldDomainList = new ArrayList<SelectItem>();
		
		// Iterar //
		this.fieldDomainList.add(new SelectItem("codigoConvenio","codigoConvenio"));
		this.fieldDomainList.add(new SelectItem("nombre","nombre"));
		this.fieldDomainList.add(new SelectItem("descripcion","descripcion"));
		this.fieldDomainList.add(new SelectItem("estado","estado"));
		this.fieldDomainList.add(new SelectItem("rutaCargaArchivos","rutaCargaArchivos"));

    }
    
    public void createConvenioWithForm() {
    }
    ...
    public void deleteConvenio(Convenio convenio) {
	    try{
	    	negocioConvenio.delete(convenio);
	    	FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_WARN, "Eliminar Convenio", "Idenfitificado por: " + "codigoConvenio: " + convenio.getCodigoConvenio());
	        FacesContext.getCurrentInstance().addMessage(null, message);
	        this.search();
        }
        catch (Exception e){
	    	FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_WARN, "No fue posible eliminar el registro", "Idenfitificado por: " + "codigoConvenio: " + convenio.getCodigoConvenio());
            FacesContext.getCurrentInstance().addMessage(null, message);
        }
    }
    ...
    public void onRowEdit(RowEditEvent event) {
	    try{
	        FacesMessage msg = new FacesMessage("Convenio Editado");
	        negocioConvenio.edit((Convenio) event.getObject());
    ...
    public void onRowCancel(RowEditEvent event) {
        FacesMessage msg = new FacesMessage("Convenio Cancelado");
        FacesContext.getCurrentInstance().addMessage(null, msg);
    }
