...
public class UsuarioManagedBean implements Serializable {
	...
	private static final Logger logger = Logger.getLogger(UsuarioManagedBean.class.getName());
    ...
    public void search() {
    	try {	
    		...
    		this.usuarioList = this.consultar(this.searchExpression);
            FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, "Busqueda", "Mostrando "+ this.usuarioList.size() + " registro(s).");
     ...
     * @return
     */
    public List<Usuario> consultar(SearchExpression search) {
        return negocioUsuario.consultar(search);
    }
    ...
    private void setFieldDomainList() {
    	this.fieldList = new ArrayList<String>(Arrays.asList(Usuario.getAtributosEntidadUsuario()));
		this.fieldDomainList = new ArrayList<SelectItem>();
		
		// Iterar //
		this.fieldDomainList.add(new SelectItem("login","login"));
		this.fieldDomainList.add(new SelectItem("pwd","pwd"));
		this.fieldDomainList.add(new SelectItem("rol","rol"));
    ...
    public void createUsuarioWithForm() {
    }
    ...
    public void deleteUsuario(Usuario usuario) {
	    try{
	    	negocioUsuario.delete(usuario);
	    	FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_WARN, "Eliminar Usuario", "Idenfitificado por: " + "login: " + usuario.getLogin());
	        FacesContext.getCurrentInstance().addMessage(null, message);
	        this.search();
        }
        catch (Exception e){
	    	FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_WARN, "No fue posible eliminar el registro", "Idenfitificado por: " + "login: " + usuario.getLogin());
            FacesContext.getCurrentInstance().addMessage(null, message);
        }
    }
    ...
    public void onRowEdit(RowEditEvent event) {
	    try{
	        ...
	        negocioUsuario.edit((Usuario) event.getObject());
