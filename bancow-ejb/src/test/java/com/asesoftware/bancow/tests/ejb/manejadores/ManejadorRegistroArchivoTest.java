...
import static com.asesoftware.bancow.modelo.entidades.RegistroArchivo.ENTIDAD_REGISTRO_ARCHIVO_REGISTRO;
import static com.asesoftware.bancow.modelo.entidades.RegistroArchivo.ENTIDAD_REGISTRO_ARCHIVO_ESTADO;
...
import static com.asesoftware.bancow.modelo.entidades.RegistroArchivo.ENTIDAD_REGISTRO_ARCHIVO_PK_NUMERO;
import static com.asesoftware.bancow.modelo.entidades.RegistroArchivo.ENTIDAD_REGISTRO_ARCHIVO_PK_ARC_PROC_CODIGO_PROCESO;
...
/**
 * Pruebas de los métodos expuestos por el ManejadorRegistroArchivo
 *
 * @author GeneradorCRUD
 */
@RunWith(Arquillian.class)
public class ManejadorRegistroArchivoTest {
...
public static final String CM_AND = "and";
	...
	public static final String ENTIDAD_REGISTRO_ARCHIVO_PK_NUMERO = "numero";
	public static final String ENTIDAD_REGISTRO_ARCHIVO_PK_ARC_PROC_CODIGO_PROCESO = "arcProcCodigoProceso";
    ...
    private static final org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(ManejadorRegistroArchivoTest.class.getName());
	...
	private static final String[] VALORES_REGISTRO = {"Descripcion1","Descripcion2","Descripcion3"};
    
    @Inject
    private ManejadorRegistroArchivo manejador;
    ...
    //Lista de instancias de RegistroArchivo generadas aleatoriamente por PODAM
    private final List<RegistroArchivo> data = new ArrayList<>();   
    ...
    public static JavaArchive createDeployment() {
        ...
        return ShrinkWrap.create(JavaArchive.class, DEPLOY + ".jar")
                .addPackage(ManejadorRegistroArchivo.class.getPackage())
                .addPackage(ManejadorPersistencia.class.getPackage())
                .addPackage(RegistroArchivo.class.getPackage())
    ...
    public void configTest() throws Exception{
        logger.debug("Configurando Test " + ManejadorRegistroArchivoTest.class.getName());
        clearData();
        insertData();
        startTransaction();
    }
    ...
    public void eliminarTest() {
        RegistroArchivo entidad = data.get(0);
        manejador.eliminar(entidad);
        RegistroArchivo deleted = em.find(RegistroArchivo.class, entidad.getRegistroArchivoPK());
    ...
    public void eliminarPorIdTest() {
        RegistroArchivo entidad = data.get(0);
        manejador.eliminarPorId(entidad.getRegistroArchivoPK());
        RegistroArchivo deleted = em.find(RegistroArchivo.class,entidad.getRegistroArchivoPK());
        Assert.assertNull(deleted);        
    }
    ...
    public void crearTest() {     
        RegistroArchivo entidad = generarRegistroArchivo();
        manejador.crear(entidad);
        RegistroArchivo creada = em.find(RegistroArchivo.class, entidad.getRegistroArchivoPK());        
    ...
    public void actualizarTest() {
        RegistroArchivo entidad = data.get(0);
        RegistroArchivoPK idEntidad = entidad.getRegistroArchivoPK();
        RegistroArchivo entidad2 = data.get(1);
        copiarPropiedades(entidad, entidad2);
        entidad.setRegistroArchivoPK(idEntidad);
        manejador.actualizar(entidad);
        RegistroArchivo entidadDB = em.find(RegistroArchivo.class, idEntidad);
        Assert.assertEquals(entidadDB,entidad);
    }
    
    @Test
    public void consultarTodosTest() {
        List<RegistroArchivo> listaConsulta = manejador.consultarPorCriteria(obtenerSearchExpresion(null, null));

        Assert.assertEquals(listaConsulta.size(), data.size());
        for(RegistroArchivo instancia : listaConsulta){
    ...
    public void consultarConFiltroPKTest() {
		...
		values1.add(entidad.getRegistroArchivoPK().getNumero().toString());
		filtros.add(new SearchExpressionCriteria(ENTIDAD_REGISTRO_ARCHIVO_PK_NUMERO, CR_EQUAL, CM_AND, values1));		
		ArrayList<String> values2 = new ArrayList<>();
		values1.add(entidad.getRegistroArchivoPK().getArcProcCodigoProceso().toString());
		filtros.add(new SearchExpressionCriteria(ENTIDAD_REGISTRO_ARCHIVO_PK_ARC_PROC_CODIGO_PROCESO, CR_EQUAL, CM_AND, values1));		
		searchExpression = obtenerSearchExpresion(filtros, null);
	    List<RegistroArchivo> listaConsulta = manejador.consultarPorCriteria(searchExpression);
    ...
    public void consultarConDosFiltrosTest() {        
        ...
        values.add(entidad.getRegistro());
        filtros.add(new SearchExpressionCriteria(ENTIDAD_REGISTRO_ARCHIVO_REGISTRO, CR_EQUAL, CM_AND, values));
        
  
        ArrayList<String> values1 = new ArrayList<>();
        values1.add(entidad.getEstado());
        filtros.add(new SearchExpressionCriteria(ENTIDAD_REGISTRO_ARCHIVO_ESTADO, CR_EQUAL, CM_AND, values1));
    ...
    public void consultarConOrdenamientoTest() {		
		SearchExpression searchExpression;
        List<RegistroArchivo> ordenarLista = data;
        Collections.sort(ordenarLista, new RegistroComparator());
        List<SearchExpressionOrder> ordenar = new ArrayList<>();
        ordenar.add(new SearchExpressionOrder(ENTIDAD_REGISTRO_ARCHIVO_REGISTRO, OR_ASC));
        searchExpression = obtenerSearchExpresion(null, ordenar);
        List<RegistroArchivo> consultarLista = manejador.consultarPorCriteria(searchExpression);
    ...
    private void insertData()  {        
        try {            
            ...
            for (int i = 0; i < REGISTROS_A_CREAR; i++) {            
                RegistroArchivo entity = generarRegistroArchivo();                       
                //Persiste el objeto en base de datos
                em.persist(entity);
                //Se añade a la lista del oráculo
                data.add(entity);
                logger.debug("Se inserto RegistroArchivo: " + entity.toString());
            }
            utx.commit();
    ...
    /**
     * Genera una nueva instancia de RegistroArchivo cuyo id no se encuentre ya en la base de datos.
     * 
     * @param factory La fábrica podam para generar POJOS
     * @return Una instancia de categoría
     */
    private RegistroArchivo generarRegistroArchivo(){
        RegistroArchivo entity = null;
        while(entity==null){
            entity = factory.manufacturePojo(RegistroArchivo.class);
            // protected region atributos adicionales on begin
            // Escriba en esta sección sus modificaciones

            // protected region atributos adicionales end
            //Este valor se setea para el test de agrupamiento
            entity.setRegistro(obtenerRegistroAleatoria());
            if( em.find(RegistroArchivo.class, entity.getRegistroArchivoPK())!=null ){
                entity = null;
     ...
     * la entidad RegistroArchivo, también elimina la lista correspondiente a estos
    ...
    private void clearData() throws TestException {        
        try {
            utx.begin();
            em.joinTransaction();
            em.createQuery("DELETE FROM RegistroArchivo").executeUpdate();
            data.clear();
            utx.commit();
    ...
    private static class RegistroComparator implements Comparator<RegistroArchivo> {
    
        @Override
        public int compare(RegistroArchivo instancia1, RegistroArchivo instancia2) {
            return instancia1.getRegistro().compareTo(instancia2.getRegistro());
        }
    }
    
	private static String  obtenerRegistroAleatoria(){
        Random generator = new Random();
        int index = generator.nextInt(VALORES_REGISTRO.length);
        
        return VALORES_REGISTRO[index];
    }
