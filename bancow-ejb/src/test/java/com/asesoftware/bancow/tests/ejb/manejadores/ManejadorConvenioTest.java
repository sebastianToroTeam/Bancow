...
import static com.asesoftware.bancow.modelo.entidades.Convenio.ENTIDAD_CONVENIO_NOMBRE;
import static com.asesoftware.bancow.modelo.entidades.Convenio.ENTIDAD_CONVENIO_DESCRIPCION;

import com.asesoftware.bancow.modelo.entidades.Convenio;

import static com.asesoftware.bancow.modelo.entidades.Convenio.ENTIDAD_CONVENIO_PK;
...
/**
 * Pruebas de los métodos expuestos por el ManejadorConvenio
 *
 * @author GeneradorCRUD
 */
@RunWith(Arquillian.class)
public class ManejadorConvenioTest {
...
public static final String CM_AND = "and";
    ...
    private static final org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(ManejadorConvenioTest.class.getName());
    ...
    @Inject
    private ManejadorConvenio manejador;
    ...
    //Lista de instancias de Convenio generadas aleatoriamente por PODAM
    private final List<Convenio> data = new ArrayList<>();   
    ...
    public static JavaArchive createDeployment() {
        ...
        return ShrinkWrap.create(JavaArchive.class, DEPLOY + ".jar")
                .addPackage(ManejadorConvenio.class.getPackage())
                .addPackage(ManejadorPersistencia.class.getPackage())
                .addPackage(Convenio.class.getPackage())
    ...
    public void configTest() throws Exception{
        logger.debug("Configurando Test " + ManejadorConvenioTest.class.getName());
        clearData();
        insertData();
        startTransaction();
    }
    ...
    public void eliminarTest() {
        Convenio entidad = data.get(0);
        manejador.eliminar(entidad);
        Convenio deleted = em.find(Convenio.class, entidad.getCodigoConvenio());
    ...
    public void eliminarPorIdTest() {
        Convenio entidad = data.get(0);
        manejador.eliminarPorId(entidad.getCodigoConvenio());
        Convenio deleted = em.find(Convenio.class,entidad.getCodigoConvenio());
        Assert.assertNull(deleted);        
    }
    ...
    public void crearTest() {     
        Convenio entidad = generarConvenio();
        manejador.crear(entidad);
        Convenio creada = em.find(Convenio.class, entidad.getCodigoConvenio());        
    ...
    public void actualizarTest() {
        Convenio entidad = data.get(0);
        String idEntidad = entidad.getCodigoConvenio();
        Convenio entidad2 = data.get(1);
        copiarPropiedades(entidad, entidad2);
        entidad.setCodigoConvenio(idEntidad);
        manejador.actualizar(entidad);
        Convenio entidadDB = em.find(Convenio.class, idEntidad);
        Assert.assertEquals(entidadDB,entidad);
    }
    
    @Test
    public void consultarTodosTest() {
        List<Convenio> listaConsulta = manejador.consultarPorCriteria(obtenerSearchExpresion(null, null));

        Assert.assertEquals(listaConsulta.size(), data.size());
        for(Convenio instancia : listaConsulta){
    ...
    public void consultarConFiltroPKTest() {
		...
		ArrayList<String> values = new ArrayList<>();
       	values.add(entidad.getCodigoConvenio().toString());
       	filtros.add(new SearchExpressionCriteria(ENTIDAD_CONVENIO_PK, CR_EQUAL, CM_AND, values));
		searchExpression = obtenerSearchExpresion(filtros, null);
	    List<Convenio> listaConsulta = manejador.consultarPorCriteria(searchExpression);
    ...
    public void consultarConDosFiltrosTest() {        
        ...
        List<SearchExpressionCriteria> filtros = new ArrayList<>();
        ArrayList<String> values = new ArrayList<>();
        values.add(entidad.getNombre());
        filtros.add(new SearchExpressionCriteria(ENTIDAD_CONVENIO_NOMBRE, CR_EQUAL, CM_AND, values));
        
  
        ArrayList<String> values1 = new ArrayList<>();
        values1.add(entidad.getDescripcion());
        filtros.add(new SearchExpressionCriteria(ENTIDAD_CONVENIO_DESCRIPCION, CR_EQUAL, CM_AND, values1));
    ...
    public void consultarConOrdenamientoTest() {		
		SearchExpression searchExpression;
        List<Convenio> ordenarLista = data;
        Collections.sort(ordenarLista, new NombreComparator());
        List<SearchExpressionOrder> ordenar = new ArrayList<>();
        ordenar.add(new SearchExpressionOrder(ENTIDAD_CONVENIO_NOMBRE, OR_ASC));
        searchExpression = obtenerSearchExpresion(null, ordenar);
        List<Convenio> consultarLista = manejador.consultarPorCriteria(searchExpression);
    ...
    private void insertData()  {        
        try {            
            ...
            for (int i = 0; i < REGISTROS_A_CREAR; i++) {            
                Convenio entity = generarConvenio();                       
                //Persiste el objeto en base de datos
                em.persist(entity);
                //Se añade a la lista del oráculo
                data.add(entity);
                logger.debug("Se inserto Convenio: " + entity.toString());
            }
            utx.commit();
    ...
    /**
     * Genera una nueva instancia de Convenio cuyo id no se encuentre ya en la base de datos.
     * 
     * @param factory La fábrica podam para generar POJOS
     * @return Una instancia de categoría
     */
    private Convenio generarConvenio(){
        Convenio entity = null;
        while(entity==null){
            entity = factory.manufacturePojo(Convenio.class);
            // protected region atributos adicionales on begin
            // Escriba en esta sección sus modificaciones
            ...
            if( em.find(Convenio.class, entity.getCodigoConvenio())!=null ){
                entity = null;
     ...
     * la entidad Convenio, también elimina la lista correspondiente a estos
    ...
    private void clearData() throws TestException {        
        try {
            utx.begin();
            em.joinTransaction();
            em.createQuery("DELETE FROM Convenio").executeUpdate();
            data.clear();
            utx.commit();
    ...
    private static class NombreComparator implements Comparator<Convenio> {
    
        @Override
        public int compare(Convenio instancia1, Convenio instancia2) {
